//! SIMD difference: sorted `u16` arrays and whole-bitmap words.
...
unsafe fn diff_neon(a: &[u16], b: &[u16], out: &mut [u16]) -> usize {
    diff_scan::<8>(a, b, out, |b, f, v| unsafe { window_has_neon(b, f, v) })
}
...
unsafe fn diff_sse2(a: &[u16], b: &[u16], out: &mut [u16]) -> usize {
    diff_scan::<8>(a, b, out, |b, f, v| unsafe { window_has_sse2(b, f, v) })
}

#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn diff_avx2(a: &[u16], b: &[u16], out: &mut [u16]) -> usize {
    diff_scan::<16>(a, b, out, |b, f, v| unsafe { window_has_avx2(b, f, v) })
}
...
pub fn andnot(dst: &mut Bitmap, src: &Bitmap) {
    ...
    #[cfg(target_arch = "aarch64")]
    unsafe {
        return fold_neon(dst, src, |a, b| std::arch::aarch64::vbicq_u64(a, b));
    }
    #[allow(unreachable_code)]
...
pub fn andnot_count(dst: &mut Bitmap, src: &Bitmap) -> u32 {
    ...
    #[cfg(target_arch = "aarch64")]
    unsafe {
        return fold_count_neon(dst, src, |a, b| std::arch::aarch64::vbicq_u64(a, b));
    }
    #[allow(unreachable_code)]
