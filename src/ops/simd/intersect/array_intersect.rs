...
use crate::ops::simd::common::MERGE_MAX_RATIO;
...
unsafe fn intersect_merge_neon(a: &[u16], b: &[u16], out: &mut [u16]) -> usize {
    ...
    loop {
        ...
        if bits == 0 {
            ...
            let shuf = vld1q_u8(crate::ops::simd::common::COMPACT[bits].as_ptr());
...
unsafe fn intersect_merge_sse(a: &[u16], b: &[u16], out: &mut [u16]) -> usize {
    ...
    loop {
        ...
        if bits == 0 {
            ...
            let shuf = _mm_loadu_si128(crate::ops::simd::common::COMPACT[bits].as_ptr().cast());
